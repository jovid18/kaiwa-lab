...
이 스킬 디렉토리에 내장된 pull 스크립트(`scripts/pull.mjs`)를 호출하는 것이 전부다 —
직접 fetch하거나 HTML을 손으로 변환하지 말 것.

> 전제: Node 20+ (외부 패키지 불필요). `.claude/skills/note-pull/.env`에 유효한
> note 세션 쿠키(`NOTE_SESSION_V5`)가 있어야 한다. 없거나 만료됐으면 아래 실패 대응 참고.
...
2. 리포 루트에서 실행:
   ...
   ```bash
   node .claude/skills/note-pull/scripts/pull.mjs "<url>"
   ```

   - 출력 디렉토리는 기본이 `diary/`라서 `--dir` 지정이 필요 없다.
...
3. **날짜 출처 검증.** 출력의 `날짜: ... (출처: ...)`를 확인한다. note 글 제목을 `YYYY-MM-DD`로
   ...
   사용자에게 올바른 날짜를 물어 `--date YYYY-MM-DD`를 추가해 다시 실행하고,
...
## 스크립트 구성 (`scripts/`, 전부 의존성 없는 순수 Node ESM)

| 파일                 | 역할                                                 |
| -------------------- | ---------------------------------------------------- |
| `pull.mjs`           | 엔트리. URL→key 추출, 날짜 결정, frontmatter+MD 저장 |
| `note-client.mjs`    | note 비공식 API 래퍼 (getNote, currentUser)          |
| `note-to-md.mjs`     | note HTML → markdown 변환기                          |
| `refresh-cookie.mjs` | Chrome에서 세션 쿠키 추출 → `.env` 갱신 (macOS 전용) |
| `env.mjs`            | `.env` 로더 + 경로 상수                              |
...
- `401/403` 또는 인증 오류(쿠키 없음/만료):
  `node .claude/skills/note-pull/scripts/refresh-cookie.mjs` 실행
  (macOS 전용, Chrome에서 note 로그인 상태여야 함).

/**
 * Chrome 프로필에서 note 세션 쿠키(_note_session_v5)를 추출해 스킬 디렉토리 .env에 기록한다. (macOS 전용)
 * (note_mcp/scripts/refresh-cookie.ts 포팅 — 의존성 없음)
 *
 * 동작: Chrome Cookies SQLite(암호화) → macOS Keychain "Chrome Safe Storage" 키로
 * PBKDF2+AES-128-CBC 복호화 → note current_user로 검증 → .env 갱신.
 *
 * 사용: node .claude/skills/note-pull/scripts/refresh-cookie.mjs
 */
...
import { getConfig, ENV_PATH } from "./env.mjs";
...
function readEncryptedSession(profile) {
...
async function main() {
  ...
  for (const profile of profiles) {
    ...
    try {
      ...
      console.error(`  [${profile}] 복호화 실패: ${e.message}`);
      continue;
    }
    // 검증
    try {
